"""Business Pricing Module

Provides pricing and hits analysis tools:
- Current and historical price queries
- Price change tracking
- Price statistics and analytics
- Hits tracking with quality filters
- Presence with price information
"""
...
def _resolve_definition(values: Optional[List[str]]) -> Optional[List[str]]:
    ...
    Args:
        values: List of definition strings (e.g., ['sd', 'hd', '4k'])
        
    Returns:
        List of normalized definitions (e.g., ['SD', 'HD', '4K']), or None
    """
    if not values:
        return None
    resolved: List[str] = []
...
def _resolve_license(values: Optional[List[str]]) -> Optional[List[str]]:
    ...
    Args:
        values: List of license strings
        
    Returns:
        List of normalized licenses, or None
    """
    if not values:
        return None
    resolved: List[str] = []
...
def _normalize_price_filters(
    ...
    """Normalize common price filters.
    
    Args:
        country: Country name or ISO-2 code
        platform_name: Platform name
        price_type: Price type(s) as string or list
        currency: Currency code(s) as string or list

    Returns:
        Tuple of (iso, plat_name, price_type_list, currency_list)
    """
...
def _build_filters_and_params(
    ...
    """Build SQL filters and parameters for derived columns.
    
    Args:
        definition: List of video definitions to filter by
        license_: List of licenses to filter by
        
    Returns:
        Tuple of (def_filter, lic_filter, params)
    """
    params: List[str] = []
...
def _build_sql_hits_quality(
    ...
    """Generate parameterized SQL for hits with quality filters.
    
    Args:
        country: Country ISO-2 code (optional, None for global)
        uid: Title UID
        limit: Maximum results
        definition: Video definitions to filter by
        license_: Licenses to filter by
...
def tool_prices_latest(
    ...
    if need_presence:
        ...
        if plat_name:
            scopes.append("p.platform_name ILIKE %s")
