"""Business Rankings Module

Provides ranking and momentum analysis tools:
- Genre momentum tracking
- Top content rankings (global and by country)
- Rating information by UID
- Flexible filtering by platform, genre, content type, date ranges
"""
...
def get_genre_momentum(
    ...
    logger.info(f"get_genre_momentum → {ident} ⇒ {len(rows) if rows else 0} rows")
...
def get_top_by_uid(uid: str) -> List[Dict]:
    ...
    logger.info(f"get_top_by_uid → {uid} ⇒ {len(rows) if rows else 0} rows")
...
def get_top_generic(
    ...
    *,
    ...
    """Flexible top content query with automatic routing based on geography.
    
    Routes automatically:
    - country/region/countries_list → uses presence data
    - no geography → uses global data
    
    Supports filters: platform, genre, content_type
    Time window: days_back or date_from/date_to
    Year filter: currentyear/year or year_from/year_to range
    ...
    Args:
        ...
        days_back: Days to look back (optional)
        date_from: Start date (optional)
        date_to: End date (optional)
        currentyear: Filter by specific year (optional)
        year_from: Start year for range (optional)
        year_to: End year for range (optional)
        region: Region name like 'LATAM', 'EU' (optional)
        countries_list: List of country codes (optional)
        type: Alias for content_type (optional)
        year: Alias for currentyear (optional)
        
    Returns:
        List of dicts with top content and metrics
    """
