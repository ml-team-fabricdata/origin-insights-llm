...
async def get_postgres_saver():
    """
    Crea una conexión psycopg v3 ASÍNCRONA con search_path=ms
    usando las mismas credenciales del SQLConnectionManager (psycopg2),
    y la pasa a AsyncPostgresSaver.
    """
    ...
    # psycopg v3 ASYNC: fijamos search_path via options y habilitamos autocommit
    ...
    async with conn.cursor() as cur:
        await cur.execute("CREATE SCHEMA IF NOT EXISTS ms;")

    saver = AsyncPostgresSaver(conn)  
    await saver.setup()
    return saver

async def get_agent(model_api='bedrock'):
