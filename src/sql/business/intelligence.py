...
def get_platform_exclusivity_by_country(
    ...
    params = (resolved_country, resolved_platform, resolved_platform, limit_norm)
...
def catalog_similarity_for_platform(
    ...
    result = db.execute_query(sql, (resolved_platform, resolved_iso_a, resolved_platform, resolved_iso_b)) or []
    ...
    similarity_percentage = (shared * 200.0 / total_unique) if total_unique > 0 else 0.0
...
def titles_in_A_not_in_B_sql(
    *,
    ...
    params: List[Any] = [c_in] + pin_params + [c_out] + pout_params + [limit_norm]
