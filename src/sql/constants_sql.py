# constants_sql.py
# Centralized constants and safe whitelists for SQL-facing tools/builders.
# Mantener este módulo SIN dependencias de la DB.

from __future__ import annotations
from typing import Dict, Tuple, FrozenSet

# ── Schemas y tablas
SCHEMA = "ms"
META_TBL = f"{SCHEMA}.metadata_simple_all"
META_ALL = f"{SCHEMA}.new_cp_metadata_estandar"
PRES_TBL = f"{SCHEMA}.new_cp_presence"
PRICES_TBL = f"{SCHEMA}.new_cp_presence_prices"
HITS_PRESENCE_TBL = f"{SCHEMA}.hits_presence_2"
HITS_GLOBAL_TBL = f"{SCHEMA}.hits_global"

TABLE_MAP: Dict[str, str] = {
    "metadata_simple_all": META_TBL,
    "new_cp_metadata_estandar": META_ALL,
    "new_cp_presence": PRES_TBL,
    "new_cp_presence_prices": PRICES_TBL,
    "hits_presence_2": HITS_PRESENCE_TBL,
    "hits_global": HITS_GLOBAL_TBL,
}
ALLOWED_TABLES: FrozenSet[str] = frozenset(TABLE_MAP.values())

# ── Defaults globales
DEFAULT_LIMIT = 10
MAX_LIMIT = 20
DEFAULT_DAYS_BACK = 30

# ── Whitelists por tabla
CP_ALLOWED_SELECT: FrozenSet[str] = frozenset({
    "iso_alpha2", "plan_name",
    "platform_code", "platform_name", "type"
})

# For metadata queries (your existing set)
META_ALLOWED_SELECT: FrozenSet[str] = frozenset({
    "uid", "title", "type", "year", "age", "duration", "synopsis",
    "primary_genre", "primary_language", "languages",
    "primary_country", "countries", "countries_iso",
    "primary_company", "production_companies",
    "directors", "full_cast", "writers",
})

META_ALLOWED_ORDER: FrozenSet[str] = frozenset({
    "title", "type", "year", "age", "duration",
    "primary_genre", "primary_language", "countries_iso",
})

PRESENCE_ALLOWED_SELECT: FrozenSet[str] = frozenset({
    "id", "sql_unique", "enter_on", "out_on", "global_id",
    "iso_alpha2", "iso_global",
    "platform_name", "platform_code", "package_code", "package_code2",
    "content_id", "hash_unique", "uid", "type", "clean_title",
    "is_original", "is_kids", "is_local", "isbranded", "is_exclusive",
    "imdb_id", "tmdb_id", "tvdb_id",
    "duration", "content_status", "registry_status",
    "uid_updated", "created_at", "plan_name", "permalink",
    "active_episodes", "active_seasons", "active_uid",
})
PRESENCE_ALLOWED_ORDER: FrozenSet[str] = frozenset({
    "enter_on", "out_on", "created_at", "uid_updated",
    "platform_name", "platform_code", "package_code",
    "iso_alpha2", "platform_country", "type", "duration", "clean_title"
})

HITS_ALLOWED_SELECT: FrozenSet[str] = frozenset({
    "uid", "imdb", "country", "content_type", "date_hits", "hits", "week", "title", "year",
    "piracynormscore", "piracyscore", "imdbnormscore", "imdbscore",
    "twitternormscore", "twitterscore", "youtubenormscore", "youtubescore",
    "input", "tmdb_id", "cdbscore", "cdbnormscore",
    "average", "hits_relative", "currentyear", "release_date", "weeks_since_release",
})
HITS_ALLOWED_ORDER: FrozenSet[str] = frozenset({
    "date_hits", "hits", "week", "year", "currentyear", "release_date", "weeks_since_release"
})

# ── Ventana activa estándar (para tablas de presencia)
ACTIVE_SQL = (
    "p.out_on IS NULL"
)

# ── Funciones SQL permitidas (nombre → (min_args, max_args))
ALLOWED_FUNCS: Dict[str, Tuple[int, int]] = {
    "LOWER": (1, 1), "UPPER": (1, 1), "INITCAP": (1, 1),
    "COALESCE": (2, -1), "NULLIF": (2, 2), "GREATEST": (2, -1), "LEAST": (2, -1),
    "DATE_TRUNC": (2, 2), "CAST": (1, 1),
    "SUM": (1, 1), "AVG": (1, 1), "MIN": (1, 1), "MAX": (1, 1), "COUNT": (0, 1),
    "PERCENTILE_CONT": (1, 1),
}

REGION_TO_ISO2 = {
    'europe': ['AD', 'AL', 'AT', 'BA', 'BE', 'BG', 'BY', 'CH', 'CY', 'CZ', 'DE', 'DK', 'EE', 'ES', 'FI', 'FO', 'FR', 'GB', 'GI', 'GR', 'HR', 'HU', 'IE', 'IM', 'IT', 'LI', 'LT', 'LU', 'LV', 'MC', 'MD', 'ME', 'MK', 'MT', 'NL', 'NO', 'PL', 'PT', 'RO', 'RS', 'RU', 'SE', 'SI', 'SK', 'SM', 'TR', 'UA', 'VA', 'XK'],
    'eu': ['AT', 'BE', 'BG', 'CY', 'CZ', 'DE', 'DK', 'EE', 'ES', 'FI', 'FR', 'GR', 'HR', 'HU', 'IE', 'IT', 'LT', 'LU', 'LV', 'MT', 'NL', 'PL', 'PT', 'RO', 'SE', 'SI', 'SK'],
    'western_europe': ['AT', 'BE', 'CH', 'DE', 'FR', 'LI', 'LU', 'MC', 'NL'],
    'northern_europe': ['DK', 'EE', 'FI', 'FO', 'GB', 'GL', 'IE', 'IM', 'LT', 'LV', 'NO', 'SE'],
    'southern_europe': ['AD', 'AL', 'BA', 'CY', 'ES', 'GI', 'GR', 'HR', 'IT', 'ME', 'MK', 'MT', 'PT', 'RS', 'SI', 'SM', 'VA', 'XK'],
    'eastern_europe': ['BG', 'BY', 'CZ', 'HU', 'MD', 'PL', 'RO', 'RU', 'SK', 'UA'],
    'north_america': ['BM', 'CA', 'GL', 'MX', 'US'],
    'south_america': ['AR', 'BO', 'BR', 'CL', 'CO', 'EC', 'FK', 'GY', 'PE', 'PY', 'SR', 'UY', 'VE'],
    'central_america': ['BZ', 'CR', 'GT', 'HN', 'NI', 'PA', 'SV'],
    'latin_america': ['AR', 'BO', 'BR', 'CL', 'CO', 'CR', 'CU', 'DO', 'EC', 'GT', 'HN', 'MX', 'NI', 'PA', 'PE', 'PR', 'PY', 'SV', 'UY', 'VE'],
    'latam': ['AR', 'BO', 'BR', 'CL', 'CO', 'CR', 'CU', 'DO', 'EC', 'GT', 'HN', 'MX', 'NI', 'PA', 'PE', 'PR', 'PY', 'SV', 'UY', 'VE'],
    'latino_america': ['AR', 'BO', 'BR', 'CL', 'CO', 'CR', 'CU', 'DO', 'EC', 'GT', 'HN', 'MX', 'NI', 'PA', 'PE', 'PR', 'PY', 'SV', 'UY', 'VE'],
    # Comprehensive Latin America (South + Central + Caribbean Spanish/Portuguese speaking)
    'latin_america_extended': ['AR', 'BO', 'BR', 'CL', 'CO', 'CR', 'CU', 'DO', 'EC', 'GT', 'HN', 'MX', 'NI', 'PA', 'PE', 'PR', 'PY', 'SV', 'UY', 'VE', 'BZ', 'GY', 'SR', 'GF', 'AI', 'AW', 'BB', 'CW', 'DM', 'GD', 'GP', 'HT', 'JM', 'KN', 'LC', 'MQ', 'MS', 'TT', 'VG', 'VI'],
    'caribbean': ['AG', 'AI', 'AW', 'BB', 'BS', 'CU', 'CW', 'DM', 'DO', 'GD', 'GP', 'HT', 'JM', 'KN', 'LC', 'MQ', 'MS', 'PR', 'TC', 'TT', 'VG', 'VI'],
    'asia': ['AE', 'AF', 'AM', 'AZ', 'BD', 'BH', 'BN', 'BT', 'CN', 'GE', 'HK', 'ID', 'IL', 'IN', 'IQ', 'IR', 'JO', 'JP', 'KG', 'KH', 'KP', 'KR', 'KW', 'KZ', 'LA', 'LB', 'LK', 'MM', 'MN', 'MO', 'MY', 'NP', 'OM', 'PH', 'PK', 'PS', 'QA', 'RU', 'SA', 'SG', 'SY', 'TH', 'TJ', 'TL', 'TM', 'TR', 'TW', 'UZ', 'VN', 'YE'],
    'middle_east': ['AE', 'BH', 'CY', 'EG', 'IL', 'IQ', 'IR', 'JO', 'KW', 'LB', 'OM', 'PS', 'QA', 'SA', 'SY', 'TR', 'YE'],
    'africa': ['AO', 'BF', 'BI', 'BJ', 'BW', 'CD', 'CF', 'CG', 'CI', 'CM', 'CV', 'DJ', 'DZ', 'EG', 'EH', 'ER', 'ET', 'GA', 'GH', 'GM', 'GN', 'GQ', 'GW', 'KE', 'KM', 'LR', 'LS', 'LY', 'MA', 'MG', 'ML', 'MR', 'MU', 'MW', 'MZ', 'NA', 'NE', 'NG', 'RE', 'RW', 'SC', 'SD', 'SL', 'SN', 'SO', 'SS', 'ST', 'SZ', 'TD', 'TG', 'TN', 'TZ', 'UG', 'ZA', 'ZM', 'ZW'],
    'oceania': ['AS', 'AU', 'CK', 'FJ', 'FM', 'GU', 'KI', 'MH', 'MP', 'NC', 'NR', 'NZ', 'PF', 'PG', 'PW', 'SB', 'TO', 'TV', 'VU', 'WF', 'WS']
}

VALID_DEFINITIONS = {"4K", "UHD", "HD", "SD", "SD/HD"}
VALID_LICENSES = {"EST", "V", "VOD"}

DEF_ALIASES = {
    "ultra hd": "UHD", "ultrahd": "UHD", "uhd": "UHD", "2160p": "4K",
    "full hd": "HD", "fullhd": "HD", "sdhd": "SD/HD", "sd/hd": "SD/HD",
}
LIC_ALIASES = {
    "electronic sell-through": "EST", "sell-through": "EST", "est": "EST",
    "vod": "VOD", "svod": "VOD", "v": "V",
}

# Alias y normalización:
REGION_ALIASES = {
    # Unión Europea
    "ue": "eu",
    "european-union": "eu",
    "european union": "eu",
    "unión europea": "eu",
    "union europea": "eu",
    # Europa (continente)
    "europa": "europe",
    # Reino Unido (país)
    "uk": "gb",
    "great britain": "gb",
    "england": "gb",
    "scotland": "gb",
    "wales": "gb",
    "northern ireland": "gb",
    "gbr": "gb",
    "united kingdom": "gb",
    'na': 'north_america',
    'sa': 'south_america',
    'ca': 'central_america',
    'we': 'western_europe',
    'ee': 'eastern_europe',
    'ne': 'northern_europe',
    'se': 'southern_europe',
    'me': 'middle_east',
    'latin america': 'latin_america',
    'latinamerica': 'latin_america',
    'latin-america': 'latin_america',
    'latin_america': 'latin_america',
    'latam': 'latam',
    'latino america': 'latino_america',
    'latinoamerica': 'latino_america',
    'latino-america': 'latino_america',
    'america latina': 'latin_america',
    'americalatina': 'latin_america',
    'america-latina': 'latin_america',
    'la': 'latin_america',  # Be careful with this one as 'LA' could also mean Los Angeles
    'lac': 'latin_america',  # Latin America and Caribbean
    'ibero america': 'latin_america',
    'ibero-america': 'latin_america',
    'iberoamerica': 'latin_america',
    'hispanic america': 'latin_america',
    'hispanic-america': 'latin_america',
    'hispanicamerica': 'latin_america',
}


DEFAULT_MIN_TITLES = 3
DEFAULT_COUNTRY = 'US'

# Tablas para AKAs y metadata
AKAS_TABLE = "ms.akas_with_year"
PG_TRGM_SCHEMA = "ms"
METADATA_TABLE = "ms.metadata_simple_all"

# Tablas para cast y directores
CAST_TABLE = "ms.cast"
ACTED_IN_TABLE = "ms.acted_in"
DIRECTOR_TABLE = "ms.directors"
DIRECTED_TABLE = "ms.directed_by"

# Umbrales de similaridad
DEFAULT_FUZZY_THRESHOLD = 0.4
FUZZY_THRESHOLD = 0.38

# Límites de candidatos
DEFAULT_FUZZY_LIMIT = 12
MAX_CANDIDATES = 25
DEFAULT_LIMIT = 50
