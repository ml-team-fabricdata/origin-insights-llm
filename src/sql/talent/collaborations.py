...
async def find_common_titles_actor_director(
    actor_name: Union[str, List[str], Any], 
    director_name: Union[str, List[str], Any], 
    limit: int = DEFAULT_LIMIT
) -> Dict[str, Any]:
    """
    Encuentra títulos en común entre un actor y un director por nombre.
    ...
    # Validar actor
    actor_validation = await validate_actor(actor_name)
    ...
    # Validar director
    director_validation = await validate_director(director_name)
...
async def get_common_projects_actor_director_by_name(
    actor_name: Union[str, List[str], Any],
    director_name: Union[str, List[str], Any],
    limit: int = DEFAULT_LIMIT
) -> str:
    """
    Get common projects between actor and director by name with validation.
    Returns JSON string for consistency with other modules.
    """
    result = await find_common_titles_actor_director(actor_name, director_name, limit)
