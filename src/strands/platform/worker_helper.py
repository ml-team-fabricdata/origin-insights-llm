# worker_helper.py - Workers dinámicos que ejecutan tools
import sys
...
async def availability_worker(state: State) -> State:
    """Worker que ejecuta tools de disponibilidad dinámicamente"""
    
    print("[WORKER] Availability worker ejecutando...")
    
    try:
        # 1. Usar el router para seleccionar la tool
        tool_name = await route_availability_tool(state)
        print(f"[WORKER] Tool seleccionada: {tool_name}")
        ...
        if not tool_fn:
            ...
            state = add_error(state, error_msg, "availability_worker")
            state = increment_tool_calls(state, worker_name="availability_worker")
            return state
        ...
        print(f"[WORKER] Datos obtenidos: {len(new_data)} caracteres")
        
        # 5. Actualizar estado usando helpers
        state = append_to_accumulated_data(
            state, 
            new_data, 
            source=f"availability_worker/{tool_name}"
        )
        
        state = increment_tool_calls(state, worker_name="availability_worker")
        
        print(f"[WORKER] Availability worker completado (tool #{state.get('tool_calls_count')})")
        return state
        
    except Exception as e:
        print(f"[ERROR] Availability worker: {str(e)}")
        state = add_error(state, str(e), "availability_worker")
        state = increment_tool_calls(state, worker_name="availability_worker")
        return state


async def presence_worker(state: State) -> State:
    """Worker que ejecuta tools de presencia dinámicamente"""
    
    print("[WORKER] Presence worker ejecutando...")
    ...
    try:
        ...
        if not tool_fn:
            error_msg = f"Tool no encontrada: {tool_name}"
            print(f"[ERROR] {error_msg}")
            state = add_error(state, error_msg, "presence_worker")
            state = increment_tool_calls(state, worker_name="presence_worker")
            return state
        ...
        state = append_to_accumulated_data(
            state, 
            new_data, 
            source=f"presence_worker/{tool_name}"
        )
        
        state = increment_tool_calls(state, worker_name="presence_worker")
        
        print(f"[WORKER] Presence worker completado (tool #{state.get('tool_calls_count')})")
        return state
        
    except Exception as e:
        print(f"[ERROR] Presence worker: {str(e)}")
        state = add_error(state, str(e), "presence_worker")
        state = increment_tool_calls(state, worker_name="presence_worker")
        return state